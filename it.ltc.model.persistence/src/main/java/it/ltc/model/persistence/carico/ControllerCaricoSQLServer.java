...
public class ControllerCaricoSQLServer extends Dao implements IControllerModel<MCarico> {
	...
	//protected final Localizer errorMessages;
	...
	public ControllerCaricoSQLServer(String persistenceUnit) {
		...
		//errorMessages = new Localizer("messages.carico.messaggiCarico", Locale.getDefault());
	}

	@Override
	public void valida(MCarico ingresso) throws ModelValidationException {
		//String messaggioErrore = errorMessages.getText("stesso_riferimento", ingresso.getRiferimento());
		ingresso.valida();		
		//Controllo il tipo di carico
		checkTipoCarico(ingresso);
		...
		if (esistente != null)
			throw new ModelAlreadyExistentException("(Legacy) Esiste gia' un carico con lo stesso riferimento. (" + ingresso.getRiferimento() + ")");
//			throw new ModelAlreadyExistentException(errorMessages.getText("stesso_riferimento", ingresso.getRiferimento()));
		//check sul fornitore
		fornitore = daoFornitori.trovaDaCodice(ingresso.getFornitore());
		if (fornitore == null)
			throw new ModelValidationException("(Legacy) Il fornitore indicato non esiste. (fornitore: " + ingresso.getFornitore() + ", carico " + ingresso.getRiferimento() + ")");
		//check sugli articoli
		if (ingresso.getProdotti().isEmpty())
			throw new ModelValidationException("(Legacy) E' necessario inserire almeno un prodotto nel carico (" + ingresso.getRiferimento() + ")");
		//check sui magazzini
		checkMagazzino(ingresso);
		//Ripulisco le mappe
		...
		TipoIDProdotto tipo = ingresso.getTipoIdentificazioneProdotti();
		switch (tipo) {
	...
	/**
	 * Controllo sui magazzini indicati per ogni riga del carico.
	 */
	protected void checkMagazzino(MCarico ingresso) {
		for (MRigaCarico prodotto : ingresso.getProdotti()) {
			...
			if (magazzino == null) {
				...
				if (magazzino == null)
					throw new ModelValidationException("(Legacy) Il magazzino indicato non esiste. (magazzino cliente: " + prodotto.getMagazzinoCliente() + ", magazzino LTC: " + prodotto.getMagazzinoLTC() + ", riga: " + prodotto.getNumeroRiga() + ", carico: " + ingresso.getRiferimento() + ")");
			}
	...
	/**
	 * Controlla che la tipologia di carico esista per il cliente.<br>
	 * Viene verificata la tabella pakiTestaTipo.
	 */
	protected void checkTipoCarico(MCarico ingresso) {
		String codice = ingresso.getTipo();
		if (daoTipo.trovaDaCodice(codice) == null)
			throw new ModelValidationException("(Legacy) Il tipo di carico specificato non esiste. (Tipo: " + codice + ", Carico: " + ingresso.getRiferimento() + ")");
	}
	
	protected void checkPerModelloTaglia(MCarico ingresso) throws ModelValidationException {
		for (MRigaCarico prodotto : ingresso.getProdotti()) {
			...
			if (!mappaIdentificazioneArticoli.containsKey(key)) {
				...
				if (articolo == null)
					throw new ModelValidationException("(Legacy) La combinazione modello-taglia: '" + prodotto.getCodicemodello() + "-"	+ prodotto.getTaglia() + "' non è stato trovata. Il carico non sarà importato. (" + ingresso.getRiferimento() + ")");
	...
	protected void checkPerChiave(MCarico ingresso) throws ModelValidationException {
		for (MRigaCarico prodotto : ingresso.getProdotti()) {
			String key = prodotto.getChiave();
			if (!mappaIdentificazioneArticoli.containsKey(key)) {
				Articoli articolo = daoArticoli.trovaDaSKU(key);
				if (articolo == null)
					throw new ModelValidationException("(Legacy) La chiave '" + key + "' non è stato trovata. Il carico non sarà importato. (" + ingresso.getRiferimento() + ")");
				mappaIdentificazioneArticoli.put(key, articolo.getIdUniArticolo());
				mappaArticoliPerIDUnivoco.put(articolo.getIdUniArticolo(), articolo);
			}
			prodotto.setChiavelegacy(mappaIdentificazioneArticoli.get(key));
	...
	protected void checkPerBarcode(MCarico ingresso) throws ModelValidationException {
		for (MRigaCarico prodotto : ingresso.getProdotti()) {
			...
			if (!mappaIdentificazioneArticoli.containsKey(key)) {
				...
				if (articolo == null)
					throw new ModelValidationException("(Legacy) Il barcode '" + key + "' non è stato trovato. Il carico non sarà importato. (" + ingresso.getRiferimento() + ")");
				mappaIdentificazioneArticoli.put(key, articolo.getIdUniArticolo());
				mappaArticoliPerIDUnivoco.put(articolo.getIdUniArticolo(), articolo);
			}
			prodotto.setChiavelegacy(mappaIdentificazioneArticoli.get(key));
	...
	public MCarico inserisci(MCarico ingresso) throws ModelPersistenceException {
		...
		try {
			...
			t.commit();
			logger.info("Carico " + ingresso.getRiferimento() + " inserito correttamente!");
			ingresso.setId(carico.getIdTestaPaki());
		} catch (Exception e) {
			logger.error("Carico " + ingresso.getRiferimento() + " in errore: " + e.getMessage(), e);
			ingresso = null;
	...
	public PakiArticolo deserializzaDettaglio(MRigaCarico riga) throws ModelPersistenceException {
		...
		dettaglio.setQtaPaki(riga.getQuantitaDichiarata());
